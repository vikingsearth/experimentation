...
show_help() {
  cat <<'HELP'
detect-changes.sh — Find what changed since a README was last modified.

Usage:
  bash scripts/detect-changes.sh <readme-path> [--scope <path>]

Arguments:
  readme-path   Path to the README file to check.
  --scope       Override the auto-detected scope directory. By default:
                - root README → entire repo (.)
                - service README → service directory (src/<service>/)
                - generic README → parent directory of the README

Behavior:
  1. Finds the last commit that modified the README.
...
if [[ -z "$README_PATH" ]]; then
  ...
  echo "Usage: bash scripts/detect-changes.sh <readme-path> [--scope <path>]" >&2
  exit 2
fi
...
if [[ -n "$SCOPE_OVERRIDE" ]]; then
  ...
  case "$TYPE" in
    root)    SCOPE="." ;;
    service) SCOPE="$(dirname "$README_PATH")" ;;
    generic) SCOPE="$(dirname "$README_PATH")" ;;
  esac
fi
...
if [[ ! -f "$README_PATH" ]]; then
  ...
  jq -n --arg path "$README_PATH" --arg type "$TYPE" \
    '{readme_path: $path, type: $type, last_modified_commit: null, last_modified_date: null, last_modified_message: null, scope: null, changes: {added:[], modified:[], deleted:[]}, summary: {total:0, added:0, modified:0, deleted:0}, warnings: ["README file not found"]}'
  exit 1
fi

# --- Find last commit that modified the README ---
LAST_COMMIT=$(git log -1 --format="%H" -- "$README_PATH" 2>/dev/null || true)

if [[ -z "$LAST_COMMIT" ]]; then
  add_warning "No git history found for ${README_PATH}. Listing all tracked files in scope instead."
  echo "Warning: No git history for ${README_PATH}. Falling back to full scope listing." >&2

  # Fallback: list all tracked files in scope
  ADDED_FILES=$(git ls-files "$SCOPE" 2>/dev/null | grep -v "^${README_PATH}$" | jq -R . | jq -s .)
  TOTAL=$(echo "$ADDED_FILES" | jq 'length')

  jq -n \
    --arg path "$README_PATH" --arg type "$TYPE" --arg scope "$SCOPE" \
...
ADDED=$(git diff --name-only --diff-filter=A "${LAST_COMMIT}..HEAD" -- "$SCOPE" 2>/dev/null | grep -v "^${README_PATH}$" || true)
MODIFIED=$(git diff --name-only --diff-filter=M "${LAST_COMMIT}..HEAD" -- "$SCOPE" 2>/dev/null | grep -v "^${README_PATH}$" || true)
DELETED=$(git diff --name-only --diff-filter=D "${LAST_COMMIT}..HEAD" -- "$SCOPE" 2>/dev/null | grep -v "^${README_PATH}$" || true)
...
echo "Changes since last README update: ${COUNT_TOTAL} files (${COUNT_A} added, ${COUNT_M} modified, ${COUNT_D} deleted)" >&2
