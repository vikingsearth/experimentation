...
show_help() {
  cat <<'HELP'
detect-changes.sh — Find what changed since CONTRIBUTING.md was last modified.

Usage:
  bash scripts/detect-changes.sh [--scope <path>]

Options:
  --scope    Override the auto-detected scope (default: entire repo ".").

Behavior:
  1. Finds the last commit that modified CONTRIBUTING.md.
...
# --- Check file exists ---
if [[ ! -f "$DOC_PATH" ]]; then
  echo "Error: CONTRIBUTING.md not found" >&2
  jq -n --arg path "$DOC_PATH" \
    '{doc_path: $path, last_modified_commit: null, last_modified_date: null, last_modified_message: null, scope: null, changes: {added:[], modified:[], deleted:[]}, summary: {total:0, added:0, modified:0, deleted:0}, warnings: ["CONTRIBUTING.md not found"]}'
  exit 1
fi
...
if [[ -z "$LAST_COMMIT" ]]; then
  ...
  jq -n \
    --arg path "$DOC_PATH" --arg scope "$SCOPE" \
