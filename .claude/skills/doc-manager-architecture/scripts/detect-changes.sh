...
show_help() {
  cat <<'HELP'
detect-changes.sh — Find what changed since an architecture doc was last modified.

Usage:
  bash scripts/detect-changes.sh <doc-path> [--scope <path>]

Arguments:
  doc-path   Path to the architecture doc to check.
...
Behavior:
  1. Finds the last commit that modified the architecture doc.
...
classify_doc() {
  local path="$1"
  path="${path#./}"
  if [[ "$path" == "docs/designs/architecture/architecture.md" ]]; then
    echo "project"
  elif [[ "$path" =~ ^docs/designs/architecture/(.+)-architecture\.md$ ]]; then
    echo "service"
  else
    echo "project"
  fi
...
# --- Check file exists ---
if [[ ! -f "$DOC_PATH" ]]; then
  echo "Error: Architecture doc not found at ${DOC_PATH}" >&2
  jq -n --arg path "$DOC_PATH" --arg type "$TYPE" --arg svc "$SERVICE_JSON" \
    '{doc_path: $path, type: $type, service: (if $svc == "null" then null else $svc end), last_modified_commit: null, last_modified_date: null, last_modified_message: null, scope: null, changes: {added:[], modified:[], deleted:[]}, summary: {total:0, added:0, modified:0, deleted:0}, warnings: ["Architecture doc not found"]}'
  exit 1
fi
